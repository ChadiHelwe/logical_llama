...
def rename_variable(g, old_variable_name, new_variable_name):
    ...
    assert (
        new_variable_name not in g.variables()
    ), "{0} variable already in graph {1}".format(new_variable_name, g)
    ...
    for old_triple in old_triples:
        if old_triple.source == old_variable_name:
            ...
            new_triple = penman.Triple(
                new_variable_name,
                old_triple.relation,
                old_triple.target,
                inverted=old_triple.inverted,
            )
...
def rename_variables(g):
    ...
    for var in old_variable_names:
        if not VARIABLE_NAME_RE.match(var):
            while (
                "z{}".format(i) in old_variable_names
                or "z{}".format(i) in new_variable_names.values()
            ):
                i += 1
